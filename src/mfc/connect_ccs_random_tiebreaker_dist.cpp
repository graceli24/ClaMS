...
int main(int argc, char **argv) {
  ...
  ygm::utility::timer root_timer;
  {
    ...
    //     saltatlas::distance::distance_function<cls::point_t,
    //     cls::distance_t>(
    //         distance_name);
    saltatlas::distance::distance_function_type<std::span<const cls::fe_t>,
                                                cls::distance_t>
        base_dist_function =
            saltatlas::distance::distance_function<std::span<const cls::fe_t>,
                                                   cls::distance_t>(
                distance_name);
    static auto ref_distance_func =
        [&base_dist_function](
            const saltatlas::pm_feature_vector<cls::fe_t> &p0,
            const saltatlas::pm_feature_vector<cls::fe_t> &p1) {
          ...
          std::span<const cls::fe_t> p0_span = std::span(p0).subspan(1);
          std::span<const cls::fe_t> p1_span = std::span(p1).subspan(1);
