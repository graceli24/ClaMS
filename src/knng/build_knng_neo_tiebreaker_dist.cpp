...
std::pair<std::vector<id_t>, std::vector<neo_dnnd_t::point_type>> read_points(
    saltatlas::mpi::communicator &comm, std::vector<std::string> &str_paths) {
  ...
  auto parse_line_lambda = [&id_vector,
    ...
    // neo_dnnd_t::point_type converted_feature = feature;
...
auto build_knng(const clams::option_t                    &opt,
  ...
  comm.cout0() << "\n<<Read Points>>" << std::endl;
  {
    ...
    std::pair<std::vector<id_t>, std::vector<neo_dnnd_t::point_type>>
        read_data = read_points(comm, str_paths);
